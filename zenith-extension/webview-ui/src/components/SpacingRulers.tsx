...
export const SpacingRulers: React.FC<SpacingRulersProps> = ({ rect, color = '#00F0FF' }) => {
  ...
  // Note: Artboard width/height is simplified here; in a full impl we'd pull from store
  ...
  const LineSegment = ({ distance, axis, pos }: { distance: number; axis: 'x' | 'y'; pos: number }) => {
  ...
  return (
    <>
      <LineSegment distance={distances.top} axis="y" pos={0} />
      <LineSegment distance={distances.bottom} axis="y" pos={rect.y + rect.height} />
      <LineSegment distance={distances.left} axis="x" pos={0} />
      <LineSegment distance={distances.right} axis="x" pos={rect.x + rect.width} />
    </>
  );
};
