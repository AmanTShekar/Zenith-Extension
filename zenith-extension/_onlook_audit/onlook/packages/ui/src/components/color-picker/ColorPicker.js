...
const InputsRow = ({ color, onChangeEnd, onChange, }) => {
    ...
    return (<div className="z-50 grid grid-cols-[48px_1fr_1fr_1fr_46px] gap-1 text-mini">
            <div className="flex items-center justify-center gap-1 min-w-0 ">
                <label className="text-small text-foreground-primary cursor-pointer hover:text-foreground-hover bg-background-secondary border-[0.5px] border-foreground-tertiary/50 hover:bg-background-hover w-full flex rounded-sm justify-center py-[0.5px] select-none" onClick={() => mode === 'hsl'
            ? setMode('hsv')
            : mode === 'hsv'
                ? setMode('rgb')
                : mode === 'rgb'
            ...
            {mode === 'hsl' ? (<input_group_1.InputGroup className="grid grid-cols-subgrid col-span-3 gap-[1px]">
                ...
                const value = (0, utility_1.mod)(Number.parseInt(valueString) / 100, 1);
                ...
                const h = (0, utility_1.mod)(Number.parseInt(hString) / 360, 1);
                ...
                const value = (0, utility_1.mod)(Number.parseInt(valueString) / 255, 1);
            ...
            const a = (0, utility_1.mod)(Number.parseInt(aString.replace('%', '')) / 100, 1);
...
const ColorPicker = ({ color, onChangeEnd, onChange, onMouseDown, className }) => {
    ...
    const handleHueChange = (0, react_1.useCallback)((h) => {
        const newHue = (0, utility_1.mod)(h, 1);
        setActiveHue(newHue);
    ...
    return (<div className={(0, utils_1.cn)('w-[224px] flex flex-col gap-1.5 p-2', className)}>
            ...
            setActiveHue((0, utility_1.mod)(h, 1));
            const newColor = new utility_1.Color({ ...localColor, h: (0, utility_1.mod)(h, 1) });
            setLocalColor(newColor);
            onChangeEnd?.(newColor);
        }} onChange={handleHueChange} onMouseDown={(h) => {
            setActiveHue((0, utility_1.mod)(h, 1));
            setLocalColor(new utility_1.Color({ ...localColor, h: (0, utility_1.mod)(h, 1) }));
            onMouseDown?.(new utility_1.Color({ ...localColor, h: (0, utility_1.mod)(h, 1) }));
        }}/>
