...
class FileSystem {
    ...
    basePath;
    watchers = new Map();
    watcherTimeouts = new Map();
    isInitialized = false;
    constructor(rootDir) {
        this.basePath = path_1.default.resolve('/', rootDir);
    }
    get rootPath() {
    ...
    async initialize() {
        ...
        this.fs = await (0, config_1.getFS)();
        ...
        for (let i = 0; i < checkLength; i++) {
            ...
            if (byte > 127) {
                ...
                for (let j = i; j < Math.min(i + 10, checkLength); j++) {
                    ...
                    if (currentByte !== undefined && currentByte > 127)
                        highByteCount++;
                }
                if (highByteCount > 7)
                    return false;
     ...
     */
    async createFile(inputPath, content = '') {
        if (!this.fs)
            throw new Error('File system not initialized');
        const fullPath = path_1.default.join(this.basePath, inputPath);
    ...
    async moveFile(from, to) {
        if (!this.fs)
            throw new Error('File system not initialized');
        const fromPath = path_1.default.join(this.basePath, from);
        const toPath = path_1.default.join(this.basePath, to);
    ...
    async createDirectory(inputPath) {
        if (!this.fs)
            throw new Error('File system not initialized');
        const fullPath = path_1.default.join(this.basePath, inputPath);
        ...
        const readDirRecursive = async (dirPath) => {
            try {
                ...
                for (const name of names) {
                    ...
                    const entry = {
                        name,
                        path: path_1.default.relative(this.basePath, entryPath), // Remove base path
    ...
    async deleteDirectory(inputPath) {
        if (!this.fs)
            throw new Error('File system not initialized');
        const fullPath = path_1.default.join(this.basePath, inputPath);
        ...
        const copyRecursive = async (src, dest) => {
            ...
            if (stats.isDirectory()) {
                ...
                for (const entry of entries) {
                    await copyRecursive(path_1.default.join(src, entry), path_1.default.join(dest, entry));
                }
            }
            else {
    ...
    watchDirectory(inputPath, callback) {
        if (!this.fs)
            throw new Error('File system not initialized');
        const watchers = [];
        ...
        const setupWatcher = (dirPath) => {
            ...
            const watcher = this.fs.watch(dirPath, async (eventType, filename) => {
                if (!filename)
                    return;
                const relativePath = path_1.default.relative(this.basePath, dirPath);
                const filePath = path_1.default.join(relativePath, filename);
        ...
        const setupWatchersRecursive = async (dirPath) => {
    ...
    async getInfo(inputPath) {
        ...
        const fullPath = path_1.default.join(this.basePath, inputPath);
        const stats = await this.fs.promises.stat(fullPath);
        const name = path_1.default.basename(inputPath);
        return {
            path: inputPath,
            name,
            isDirectory: stats.isDirectory(),
    ...
    async listFiles(pattern = '**/*') {
        if (!this.fs)
            throw new Error('File system not initialized');
        const files = [];
        const listRecursive = async (dirPath) => {
            try {
                const entries = await this.fs.promises.readdir(dirPath);
                for (const entry of entries) {
                    const entryPath = path_1.default.join(dirPath, entry);
                    const stats = await this.fs.promises.stat(entryPath);
                    if (stats.isFile()) {
                        const relativePath = path_1.default.relative(this.basePath, entryPath);
