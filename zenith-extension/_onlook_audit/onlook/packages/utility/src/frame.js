...
exports.calculateNonOverlappingPosition = calculateNonOverlappingPosition;
function calculateNonOverlappingPosition(proposedFrame, existingFrames) {
    const SPACING = 100;
...
function hasOverlap(position, proposedFrame, existingFrames, spacing) {
    const proposed = {
        left: position.x,
        top: position.y,
    ...
    return existingFrames.some((existingFrame) => {
        if (existingFrame.id === proposedFrame.id)
            return false;
        const existing = {
...
function findBottomLeftPosition(proposedFrame, existingFrames, spacing) {
...
function getAnchorPoints(existingFrames, spacing) {
    const points = [];
    for (const frame of existingFrames) {
