...
function getInsertLocation(x, y) {
    ...
    if (isStackOrGrid) {
        ...
        return {
            ...
            targetOid: (0, ids_1.getInstanceId)(targetEl) || (0, ids_1.getOid)(targetEl) || null,
            index,
            originalIndex: index,
        };
    }
...
function findNearestBlockLevelContainer(x, y) {
    let targetEl = (0, helpers_2.getDeepElement)(x, y);
    if (!targetEl) {
        return null;
    }
    let inlineOnly = true;
...
function insertElement(element, location) {
    const targetEl = (0, helpers_1.getHtmlElement)(location.targetDomId);
    if (!targetEl) {
    ...
    switch (location.type) {
        ...
        default:
            ...
            (0, helpers_1.assertNever)(location);
    }
    const domEl = (0, helpers_2.getDomElement)(newEl, true);
    const newMap = (0, dom_1.buildLayerTree)(newEl);
    return {
        domEl,
        newMap,
    };
...
function createElement(element) {
    ...
    for (const [key, value] of Object.entries(element.styles)) {
        newEl.style.setProperty(style_1.cssManager.jsToCssProperty(key), value);
    }
...
function removeElement(location) {
    ...
    if (elementToRemove) {
        const domEl = (0, helpers_2.getDomElement)(elementToRemove, true);
        elementToRemove.style.display = 'none';
        const newMap = targetEl.parentElement ? (0, dom_1.buildLayerTree)(targetEl.parentElement) : null;
        return {
            domEl,
            newMap,
        };
