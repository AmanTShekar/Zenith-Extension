...
exports.TailwindColorEditorMockup = TailwindColorEditorMockup;
const react_1 = __importStar(require("react"));
const utility_1 = require("@onlook/utility");
const color_picker_1 = require("@onlook/ui/color-picker");
const icons_1 = require("@onlook/ui/icons");
const color_swatch_group_1 = require("../../landing-page/color-swatch-group");
const toolbar_button_1 = require("@/app/project/[id]/_components/editor-bar/toolbar-button");
const separator_1 = require("@/app/project/[id]/_components/editor-bar/separator");
function TailwindColorEditorMockup() {
    const [color, setColor] = (0, react_1.useState)(() => utility_1.Color.from('#0088FF').withAlpha(1));
    // Local aliases to satisfy JSX typing in this mock file
    const ImageIcon = icons_1.Icons.Image;
    const CloseIcon = icons_1.Icons.CrossS;
    ...
    return (<div className="w-full h-100 bg-background-onlook/80 rounded-lg relative overflow-hidden">
      ...
      <div className="absolute top-24 left-[200px] z-10 w-[640px] h-[420px] rounded-[28px] border border-foreground-primary/10 overflow-hidden shadow-[0_30px_80px_-20px_rgba(0,0,0,0.45)]" style={{
        ...
        {(() => {
            ...
            const complementary = new utility_1.Color({ ...color, h: (color.h + 0.5) % 1, a: 1 }).toHex();
            const triadic = new utility_1.Color({ ...color, h: (color.h + 0.33) % 1, a: 1 }).toHex();
            const split1 = new utility_1.Color({ ...color, h: (color.h + 0.42) % 1, a: 1 }).toHex();
            const split2 = new utility_1.Color({ ...color, h: (color.h + 0.58) % 1, a: 1 }).toHex();
            const primaryLight = new utility_1.Color({ ...color, v: Math.min(1, color.v * 1.12), a: 1 }).toHex();
            const primaryDark = new utility_1.Color({ ...color, v: Math.max(0, color.v * 0.82), a: 1 }).toHex();
            ...
            return (<>
              ...
              <div className="absolute -inset-40 blur-3xl opacity-90" style={{
              ...
              <div className="absolute inset-0" style={{ filter: 'blur(16px) saturate(110%)' }}>
                ...
                <div className="absolute inset-0 mix-blend-screen opacity-[0.14]" style={{
                ...
                <div className="absolute inset-0 mix-blend-screen opacity-[0.11]" style={{
                ...
                <div className="absolute inset-0 mix-blend-screen opacity-[0.1]" style={{
              ...
              <div className="absolute w-[55%] h-[55%] left-[-5%] top-[-5%] rounded-[40%] mix-blend-screen opacity-90" style={{ background: primary, filter: 'blur(40px)', animation: 'blobFloat1 16s ease-in-out infinite' }}/>
              <div className="absolute w-[45%] h-[45%] right-[-8%] top-[5%] rounded-[45%] mix-blend-screen opacity-90" style={{ background: complementary, filter: 'blur(48px)', animation: 'blobFloat2 20s ease-in-out -2s infinite' }}/>
              <div className="absolute w-[60%] h-[55%] left-[10%] bottom-[-10%] rounded-[45%] mix-blend-screen opacity-85" style={{ background: triadic, filter: 'blur(56px)', animation: 'blobFloat3 22s ease-in-out -4s infinite' }}/>
              {/* extra small energetic blobs */}
              <div className="absolute w-[24%] h-[24%] right-[12%] bottom-[12%] rounded-[50%] mix-blend-screen opacity-55" style={{ background: split1, filter: 'blur(28px)', animation: 'blobFloat4 14s ease-in-out -1s infinite' }}/>
              <div className="absolute w-[18%] h-[18%] left-[6%] top-[30%] rounded-[50%] mix-blend-screen opacity-60" style={{ background: split2, filter: 'blur(24px)', animation: 'blobFloat5 12s ease-in-out -3s infinite' }}/>
      ...
      <div className="absolute top-4 left-4 flex flex-col border-[0.5px] border-border p-1 px-1 bg-background rounded-xl backdrop-blur drop-shadow-xl z-20 overflow-hidden w-auto">
        ...
        <toolbar_button_1.ToolbarButton isOpen className="flex w-10 flex-col items-center justify-center gap-0.5">
        ...
        <toolbar_button_1.ToolbarButton className="flex items-center justify-center w-9 h-9">
          <ImageIcon className="h-4 w-4"/>
        </toolbar_button_1.ToolbarButton>
        <toolbar_button_1.ToolbarButton className="flex items-center justify-center w-9 h-9">
          <icons_1.Icons.BorderEdit className="h-4 w-4"/>
        </toolbar_button_1.ToolbarButton>
        <toolbar_button_1.ToolbarButton className="flex w-10 flex-col items-center justify-center gap-0.5">
          <icons_1.Icons.PencilIcon className="h-4 w-4"/>
          <div className="h-[4px] w-6 rounded-full bg-foreground-primary"/>
        </toolbar_button_1.ToolbarButton>
        <separator_1.InputSeparator />
        <toolbar_button_1.ToolbarButton className="flex items-center gap-1 min-w-10">
          <icons_1.Icons.Layout className="h-4 w-4 min-h-4 min-w-4 text-foreground-primary"/>
          <span className="text-small text-foreground-primary">Flex</span>
        </toolbar_button_1.ToolbarButton>
        <toolbar_button_1.ToolbarButton className="gap-1 flex items-center min-w-10">
          <icons_1.Icons.PaddingFull className="h-4 min-h-4 w-4 min-w-4"/>
          <span className="text-small text-foreground-primary">8</span>
        </toolbar_button_1.ToolbarButton>
        <toolbar_button_1.ToolbarButton className="gap-1 flex items-center min-w-10">
          <icons_1.Icons.MarginTRB className="h-4 min-h-4 w-4 min-w-4"/>
          <span className="text-small text-foreground-primary">Mixed</span>
        </toolbar_button_1.ToolbarButton>
        <separator_1.InputSeparator />
        <toolbar_button_1.ToolbarButton className="flex items-center justify-start px-4 min-w-10">
          <span className="text-regularPlus">Inter</span>
        </toolbar_button_1.ToolbarButton>
        <separator_1.InputSeparator />
        <toolbar_button_1.ToolbarButton className="flex items-center gap-1 min-w-10 px-3">
        ...
        {/* Align left */}
        <toolbar_button_1.ToolbarButton className="flex items-center justify-center min-w-9">
          {/* eslint-disable-next-line react/no-children-prop */}
          {react_1.default.createElement(icons_1.Icons.TextAlignLeft, { className: 'h-4 w-4' })}
        </toolbar_button_1.ToolbarButton>
        {/* Advanced typography */}
        <toolbar_button_1.ToolbarButton className="flex items-center justify-center min-w-9 px-2">
          <icons_1.Icons.AdvancedTypography className="h-4 w-4"/>
        </toolbar_button_1.ToolbarButton>
        <separator_1.InputSeparator />
         <toolbar_button_1.ToolbarButton className="flex items-center justify-center min-w-9">
           {react_1.default.createElement(icons_1.Icons.DotsHorizontal, { className: 'h-4 w-4' })}
         </toolbar_button_1.ToolbarButton>
        </div>
      </div>
      ...
      <div className="absolute top-18 left-5 w-[232px] bg-black/85 backdrop-blur-lg border border-foreground-primary/20 rounded-xl shadow-lg overflow-hidden z-30">
        ...
        <div className="px-1 pb-3 mt-0.5">
          <div className="flex flex-col gap-3">
            ...
            <div className="mt-3">
                <color_swatch_group_1.ColorSwatchGroup label="bunker" colorClasses={bunkerSwatches}/>
           </div>
          </div>
        </div>
      </div>
...
//# sourceMappingURL=tailwind-color-editor.js.map