...
const connect_1 = require("./_components/connect");
const finalizing_1 = require("./_components/finalizing");
const setup_1 = require("./_components/setup");
const _context_1 = require("./_context");
const steps = [
    <connect_1.ConnectGithub />,
    <setup_1.SetupGithub />,
    <finalizing_1.FinalizingGithubProject />
];
const Page = () => {
    const { currentStep } = (0, _context_1.useImportGithubProject)();
            ...
            backgroundImage: `url(${backgroundUrl})`,
                ...
                <div className="relative z-10">
                    <react_1.MotionConfig transition={{ duration: 0.5, type: 'spring', bounce: 0 }}>
                        <motion_card_1.MotionCard initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="w-[30rem] min-h-[12rem] backdrop-blur-md bg-background/30 overflow-hidden p-0">
                            <react_1.motion.div ref={ref} layout="position" className="flex flex-col">
                                <react_1.AnimatePresence mode="popLayout" initial={false}>
                                    <react_1.motion.div key={currentStep} 
    // custom={direction}
    variants={variants} initial="initial" animate="active" exit="exit">
                                        {steps[currentStep]}
                                    </react_1.motion.div>
