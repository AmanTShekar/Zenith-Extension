...
exports.OnlookInterfaceMockup = OnlookInterfaceMockup;
const icons_1 = require("@onlook/ui/icons");
const node_icon_1 = require("@onlook/ui/node-icon");
const utils_1 = require("@onlook/ui/utils");
const react_1 = __importStar(require("react"));
const design_mockup_1 = require("./design-mockup/design-mockup");
...
function NotesComponent() {
    ...
    return (<div className="bg-black/80 backdrop-blur-sm border border-foreground-secondary/20 rounded-lg p-3 w-96 min-w-64 h-fit mt-10">
            <div className="flex items-center gap-2 mb-1">
                <h3 className="text-foreground-secondary text-xs font-mono">Villainterest - Product Notes</h3>
            </div>
            <div className="space-y-2">
                {notes.map((note, index) => (<div key={index} className="flex items-start gap-2 text-[10px]">
...
function OnlookInterfaceMockup() {
    ...
    const [selectedId, setSelectedId] = (0, react_1.useState)('1.2.2.1.1');
    const [layersPanelOpen, setLayersPanelOpen] = (0, react_1.useState)(true);
    // Canvas panning state
    const [isPanning, setIsPanning] = (0, react_1.useState)(false);
    const [panOffset, setPanOffset] = (0, react_1.useState)({ x: 60, y: -30 }); // Center on first mockup (accounting for layers panel)
    ...
    return (<div className={(0, utils_1.cn)("relative w-full max-w-6xl mx-auto aspect-[16/10] rounded-xl overflow-hidden shadow-2xl border border-neutral-800 bg-background-onlook select-none -mt-10 transition-all duration-1000 ease-out", isVisible
            ? "opacity-100 translate-y-0"
            : "opacity-0 translate-y-8")}>
            <div className="absolute inset-0 flex items-start mt-30 justify-center pointer-events-none z-0 right-36 select-none gap-12" style={{
                ...
                <div className="relative flex flex-col items-center border-1 border-teal-300 rounded-sm shadow-xl shadow-black/50">
                    <div className="absolute left-1/2 -translate-x-1/2 -top-7 z-50 w-full flex flex-row items-center rounded-lg h-6 text-xs px-1 gap-2.5 backdrop-blur-lg">
                        <div className="flex-1 flex flex-row items-center gap-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-[12px] text-teal-300">
                            Home
                            <icons_1.Icons.ChevronDown className="w-4 h-4 text-teal-400 mb-0.5"/>
                        </div>
                        <button className="w-3 h-3 flex items-center justify-center cursor-pointer" tabIndex={-1} style={{ pointerEvents: 'none' }}>
                            <icons_1.Icons.DotsHorizontal className="w-3.5 h-3.5 text-teal-300"/>
                        </button>
                    </div>
                    <design_mockup_1.DesignMockup />
                </div>
                <div className="relative flex flex-col items-center border-[0.5px] border-foreground-border rounded-sm shadow-xl shadow-black/50 ml-8">
                    <div className="absolute left-1/2 -translate-x-1/2 -top-7 z-50 w-full flex flex-row items-center rounded-lg h-6 text-xs px-1 gap-2.5 backdrop-blur-lg">
                        <div className="flex-1 flex flex-row items-center gap-1.5 overflow-hidden text-ellipsis whitespace-nowrap text-[12px] text-foreground-secondary">
                            Home
                            <icons_1.Icons.ChevronDown className="w-4 h-4 text-foreground-secondary mb-0.5"/>
                        </div>
                        <button className="w-3 h-3 flex items-center justify-center cursor-pointer" tabIndex={-1} style={{ pointerEvents: 'none' }}>
                            <icons_1.Icons.DotsHorizontal className="w-3.5 h-3.5 text-foreground-secondary"/>
                        </button>
                    </div>
                    <design_mockup_1.DesignMockupMobile />
                </div>
            </div>
            {/* Top Bar */}
            <div className="grid grid-cols-3 items-center h-10 px-2.5 relative z-10">
                ...
                <div className="flex items-center gap-1 min-w-0">
                    <icons_1.Icons.OnlookLogo className="w-5 h-5 shrink-0"/>
                    <span className="text-xs text-foreground-secondary ml-1 truncate max-w-[100px]">Villainterest</span>
                    <icons_1.Icons.ChevronDown className="w-4 h-4 text-neutral-400 ml-0.5"/>
                </div>
                ...
                <div className="flex items-center gap-1.5 justify-end">
                    <button className="p-1 rounded hover:bg-neutral-800">
                        <icons_1.Icons.Reset className="w-4 h-4 text-foreground-secondary"/>
                    </button>
                    <button className="p-1 rounded hover:bg-neutral-800">
                        <icons_1.Icons.Reset className="w-4 h-4 text-foreground-secondary scale-x-[-1]"/>
                    </button>
                    <button className="bg-teal-900 border-[1px] border-teal-200 text-teal-200 text-xs px-2.5 py-1 rounded ml-2 flex flex-row items-center gap-1.5">
                        <icons_1.Icons.Globe className="w-3.5 h-3.5"/>
                        Live
                    </button>
            ...
            <div className="flex h-[calc(100%-2.5rem)] relative">
                ...
                <div className="w-14 h-full flex flex-col items-center justify-between bg-background-onlook/80 backdrop-blur-xl mr-[-4] px-2">
                    <div className="flex flex-col items-center py-4 gap-5">
                        {/* Active: Layers */}
                        <div className="flex flex-col items-center gap-0.5 rounded-md px-2 py-1.5 bg-background-tertiary/50 ring-1 ring-white/5 border-[0.5px] border-foreground-primary/20 cursor-pointer hover:bg-background-tertiary/70 transition-colors" onClick={() => setLayersPanelOpen(!layersPanelOpen)}>
                            <icons_1.Icons.Layers className="w-4.5 h-4.5 text-foreground-primary"/>
                            <p className="text-[10px] text-foreground-primary">Layers</p>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                            <icons_1.Icons.Brand className="w-4.5 h-4.5 text-foreground-secondary"/>
                            <p className="text-[10px] text-foreground-secondary">Brand</p>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                            <icons_1.Icons.File className="w-4.5 h-4.5 text-foreground-secondary"/>
                            <p className="text-[10px] text-foreground-secondary">Pages</p>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                            <icons_1.Icons.Image className="w-4.5 h-4.5 text-foreground-secondary"/>
                            <p className="text-[10px] text-foreground-secondary">Assets</p>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                            <icons_1.Icons.Component className="w-4.5 h-4.5 text-foreground-secondary"/>
                            <p className="text-[10px] text-foreground-secondary">Elements</p>
                        </div>
                        <div className="flex flex-col items-center gap-1">
                            <icons_1.Icons.ViewGrid className="w-4.5 h-4.5 text-foreground-secondary"/>
                    ...
                    <div className="flex flex-row items-center justify-center w-full mb-6">
                        <icons_1.Icons.QuestionMarkCircled className="w-4.5 h-4.5 text-foreground-secondary"/>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col border-[0.5px] border-border p-1 px-1 bg-black/60 backdrop-blur-2xl rounded-lg backdrop-blur drop-shadow-xl overflow-hidden pointer-events-auto">
                        <div className="flex flex-row gap-0.5">
                            ...
                            <div className="h-8 w-8 flex items-center justify-center rounded-md border border-transparent bg-background-tertiary/50 text-foreground-primary">
                                <icons_1.Icons.CursorArrow className="w-4 h-4 text-foreground-primary"/>
                            </div>
                            {/* Unselected icons */}
                            <div className="h-8 w-8 flex items-center justify-center rounded-md border border-transparent text-foreground-tertiary hover:text-foreground-hover hover:bg-background-tertiary/50">
                                <icons_1.Icons.Hand className="w-4 h-4 text-foreground-tertiary"/>
                            </div>
                            <div className="h-8 w-8 flex items-center justify-center rounded-md border border-transparent text-foreground-tertiary hover:text-foreground-hover hover:bg-background-tertiary/50">
                                <icons_1.Icons.Square className="w-4 h-4 text-foreground-tertiary"/>
                            </div>
                            <div className="h-8 w-8 flex items-center justify-center rounded-md border border-transparent text-foreground-tertiary hover:text-foreground-hover hover:bg-background-tertiary/50">
                                <icons_1.Icons.Text className="w-4 h-4 text-foreground-tertiary"/>
                            </div>
                            <div className="h-8 w-8 flex items-center justify-center rounded-md border border-transparent text-foreground-tertiary hover:text-foreground-hover hover:bg-background-tertiary/50">
                                <icons_1.Icons.Terminal className="w-4 h-4 text-foreground-tertiary"/>
                            </div>
                        </div>
                    </div>
                </div>
                {/* Layers Side Panel (mini mockup) */}
                {layersPanelOpen && (<div className="w-52 h-full px-1 pt-1">
                ...
                return (<div key={layer.id} className={(0, utils_1.cn)('flex items-center h-5.25 px-1.5 cursor-pointer transition-colors select-none text-xs', 
                    // Component styling
                    isComponent && !layer.isInstance && !isHovered && 'text-purple-600 dark:text-purple-300', isComponent && !layer.isInstance && isHovered && 'text-purple-500 bg-purple-800/50 dark:text-purple-200', isComponent && !layer.isInstance && isSelected && 'bg-purple-500 dark:bg-purple-500/90 text-white dark:text-primary', 
                    // Instance styling
                    layer.isInstance && isSelected && 'text-purple-100 dark:text-purple-100 bg-purple-700/70 dark:bg-purple-500/50', layer.isInstance && !isSelected && 'text-purple-500 dark:text-purple-300', layer.isInstance && !isSelected && isHovered && 'text-purple-800 dark:text-purple-200 bg-purple-400/30 dark:bg-purple-900/60', 
                    // Regular selection styling
                    !isComponent && !layer.isInstance && isSelected && 'bg-[#FA003C] dark:bg-[#FA003C]/90 text-white dark:text-primary', !isComponent && !layer.isInstance && isHovered && !isSelected && 'bg-background-onlook text-foreground-onlook', !isComponent && !layer.isInstance && !isSelected && !isHovered && 'text-foreground-onlook', 
                    // Rounded corners
                    isHovered && !isSelected && 'rounded', isSelected && 'rounded')} onMouseEnter={() => setHoveredId(layer.id)} onMouseLeave={() => setHoveredId(null)} onClick={() => setSelectedId(layer.id)} style={{ userSelect: 'none' }}>
                                                <div style={{ width: `${layer.level * 16}px` }}/>
                                                <node_icon_1.NodeIcon iconClass="w-3.5 h-3.5 mr-1.5" tagName={layer.tagName}/>
                ...
                <div className="flex-1 flex flex-col items-center justify-start relative cursor-grab active:cursor-grabbing" onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseLeave}/>
                <div className="w-64 bg-black/70 backdrop-blur-2xl border-l border-t rounded-tl-xl border-foreground-border flex flex-col justify-end p-0 relative">
                    <div className="absolute inset-0 flex flex-col">
                        <div className="flex items-center justify-between h-9 px-0.5 border-b border-foreground-border z-20">
                            ...
                            <div className="flex items-center justify-between h-9 px-0.5 border-b border-foreground-border z-20">
                                <button className="text-xs text-foreground-secondary px-2 py-1 rounded hover:text-foreground-primary flex flex-row items-center gap-1">
                                    <icons_1.Icons.Plus className="w-3.5 h-3.5 text-foreground-secondary"/>
                                </button>
                            </div>
                        </div>
            ...
            return null;
                            ...
                            <div className="border-t border-foreground-primary/10 px-2.5 py-2 flex flex-col items-start gap-1">
                                <textarea value={displayedText} readOnly className="flex-1 text-foreground-primary placeholder-foreground-tertiary text-xs px-0.5 pt-2 h-20 mb-5 w-full resize-none rounded-lg outline-none bg-transparent" placeholder="Type a message..." rows={3} maxLength={PRESET_SENTENCE.length} disabled/>
                                <div className="flex flex-row items-center justify-between w-full gap-2">
                                    ...
                                    <div className="flex flex-row gap-1">
                                        ...
                                        <button className={`px-2 py-2 rounded-full cursor-pointer ${currentIndex === PRESET_SENTENCE.length ? 'bg-foreground-primary' : 'bg-foreground-onlook'}`} disabled>
                                            <icons_1.Icons.ArrowRight className="h-3.5 w-3.5 text-background-secondary"/>
                                        </button>
                                    </div>
                                </div>
                            </div>
...
//# sourceMappingURL=onlook-interface-mockup.js.map