...
function RevisionHistory() {
    ...
    return (<div className="flex flex-col gap-6">
            <div className="w-full h-100 bg-background-onlook/80 rounded-lg relative overflow-hidden">
                ...
                <div className="w-100 h-100 rounded-xl overflow-hidden absolute right-[-150px] top-10 flex flex-col items-center justify-start bg-black/85 backdrop-blur-2xl border-[0.5px] border-foreground-primary/20 shadow-lg z-40">
                    ...
                    <div className="w-full h-full overflow-y-auto px-0 py-2 flex flex-col gap-2">
                        ...
                        <div className="flex flex-col gap-0 mb-2 border-b-[0.5px] border-foreground-primary/20 pb-2">
                            {todayVersions.map((v, idx) => (<VersionRow key={v.title} title={v.title} subtitle={v.subtitle} selected={selectedVersionIdx === idx} onClick={() => handleVersionSelect(idx)}/>))}
                        </div>
                        {/* Yesterday */}
                ...
                <div key={selectedVersionIdx} className={`w-100 h-100 rounded-sm overflow-hidden absolute left-7 top-20 flex flex-col items-center justify-start border-[0.5px] border-foreground-primary/20 shadow-lg z-10 transition-all duration-200 ease-in-out relative ${demoColors[selectedVersionIdx]} transform ${isAnimating ? 'scale-95' : 'scale-100'} ${isFading ? 'opacity-50' : 'opacity-100'}`}>
                    {demoImages[displayedImageIdx] && (<img src={demoImages[displayedImageIdx]} alt="Site version preview" className={`absolute inset-0 w-full h-full object-cover transition-all duration-200 ease-in-out ${isFading ? 'opacity-0' : 'opacity-100'}`} style={{ zIndex: 2 }}/>)}
                </div>
            </div>
            <div className="flex flex-row items-start gap-8 w-full">
                {/* Icon + Title */}
                <div className="flex flex-col items-start w-1/2">
                    <div className="mb-2"><icons_1.Icons.CounterClockwiseClock className="w-6 h-6 text-foreground-primary"/></div>
...
//# sourceMappingURL=revision-history.js.map