...
const color_popover_1 = require("./color-popover");
const BrandPalletGroup = ({ title, colors, theme, onRename, onDelete, onColorChange, onColorChangeEnd, onDuplicate, isDefaultPalette = false, }) => {
    ...
    const getColorValue = (color) => {
        return theme === assets_1.SystemTheme.DARK
    ...
    const handleViewInCode = (color) => {
        ...
        const line = theme === assets_1.SystemTheme.DARK ? color.line.css?.darkMode : color.line.css?.lightMode;
    ...
    return (<div className="flex flex-col gap-1 group/palette">
            <div className="flex justify-between items-center">
                {!isDefaultPalette && isRenaming ? (<color_name_input_1.ColorNameInput initialName={title} onSubmit={(newName) => {
                ...
                {!isDefaultPalette && (<dropdown_menu_1.DropdownMenu modal={false}>
            ...
            <div className="flex flex-col gap-2">
                <div className="grid grid-cols-6 gap-1">
                    {colors ? (colors.map((color, index) => (<div key={`${title}-${index}`} className="relative group">
                                {editingColorIndex === index ? (<color_popover_1.ColorPopover color={utility_1.Color.from(getColorValue(color))} brandColor={color.name} onClose={() => setEditingColorIndex(null)} onColorChange={(newColor, newName) => handleColorChange(index, newColor, newName)} onColorChangeEnd={(newColor, newName) => handleColorChangeEnd(index, newColor, newName)} isDefaultPalette={isDefaultPalette} existedName={existedName}/>) : (<>
...
//# sourceMappingURL=color-pallet-group.js.map