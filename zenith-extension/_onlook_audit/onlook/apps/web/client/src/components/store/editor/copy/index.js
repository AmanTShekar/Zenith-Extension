...
const helpers_1 = require("../history/helpers");
class CopyManager {
    editorEngine;
    copied = null;
    ...
    async copy() {
        ...
        const targetEl = (await frameData.view.getActionElement(selectedEl.domId));
        if (!targetEl) {
            console.error('Failed to copy element');
            return;
        }
    ...
    async paste() {
        ...
        const newOid = (0, utility_1.createOid)();
        const newDomId = (0, utility_1.createDomId)();
        const action = {
            type: 'insert-element',
            targets: targets,
            element: (0, helpers_1.getCleanedElement)(this.copied.element, newDomId, newOid),
            location,
            editText: null,
            pasteParams: {
                oid: newOid,
    ...
    async getInsertLocation(selectedEl) {
        ...
        if (insertAsSibling) {
            const location = await frameData.view.getActionLocation(selectedEl.domId);
            if (!location) {
