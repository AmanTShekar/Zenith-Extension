...
class AstManager {
    ...
    async processNodeForMap(frameId, node) {
        if (!node.oid) {
        ...
        if (frameData.view) {
            frameData.view.setElementType(node.domId, metadata.dynamicType || null, metadata.coreElementType || null);
        }
        else {
    ...
    async findNodeInstance(frameId, originalNode, node, metadata, branchData) {
        ...
        if (parentMetadata.component !== metadata.component) {
            ...
            const index = childrenWithSameOid.findIndex((child) => child.domId === originalNode.domId);
            if (index === -1) {
                console.warn('Failed to findNodeInstance: Original node not found in children with same OID');
                return;
            }
            const res = await (0, parser_1.getTemplateNodeChild)(parentMetadata.code, metadata, index);
            if (res) {
                originalNode.instanceId = res.instanceId;
                originalNode.component = res.component;
                this.updateElementInstance(frameId, originalNode.domId, res.instanceId, res.component);
            }
            else {
    ...
    updateElementInstance(frameId, domId, instanceId, component) {
        const frameData = this.editorEngine.frames.get(frameId);
        if (!frameData?.view) {
