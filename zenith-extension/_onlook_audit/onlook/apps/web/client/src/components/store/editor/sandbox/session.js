...
class SessionManager {
    ...
    constructor(branch, errorManager) {
        this.branch = branch;
        this.errorManager = errorManager;
        (0, mobx_1.makeAutoObservable)(this);
    }
    async start(sandboxId, userId) {
        const MAX_RETRIES = 3;
                ...
                providerOptions: {
                    codesandbox: {
                        ...
                        getSession: async (sandboxId, userId) => {
                            return client_1.api.sandbox.start.mutate({ sandboxId });
                        },
                    },
                },
            });
    ...
    async createTerminalSessions(provider) {
        const task = new terminal_1.CLISessionImpl('server', terminal_1.CLISessionType.TASK, provider, this.errorManager);
        this.terminalSessions.set(task.id, task);
        const terminal = new terminal_1.CLISessionImpl('terminal', terminal_1.CLISessionType.TERMINAL, provider, this.errorManager);
    ...
    async disposeTerminal(id) {
        ...
        if (terminal) {
            if (terminal.type === terminal_1.CLISessionType.TERMINAL) {
    ...
    async hibernate(sandboxId) {
        await client_1.api.sandbox.hibernate.mutate({ sandboxId });
    }
    async reconnect(sandboxId, userId) {
        try {
            if (!this.provider) {
    ...
    async restartProvider(sandboxId, userId) {
        if (!this.provider) {
            return;
        }
    ...
    async runCommand(command, streamCallback, ignoreError = false) {
        try {
            if (!this.provider) {
