...
exports.adminProcedure = exports.protectedProcedure = exports.publicProcedure = exports.createTRPCRouter = exports.createCallerFactory = exports.createTRPCContext = void 0;
...
const createTRPCContext = async (opts) => {
    ...
    if (error) {
        throw new server_2.TRPCError({ code: 'UNAUTHORIZED', message: error.message });
    }
    return {
        db: client_1.db,
        supabase,
 ...
 */
exports.protectedProcedure = t.procedure.use(timingMiddleware).use(({ ctx, next }) => {
    if (!ctx.user) {
 ...
 * @see https://trpc.io/docs/procedures
 */
exports.adminProcedure = t.procedure.use(timingMiddleware).use(({ ctx, next }) => {
    if (!ctx.user) {
        throw new server_2.TRPCError({ code: 'UNAUTHORIZED' });
    }
    if (!ctx.user.email) {
    ...
    const adminSupabase = (0, admin_1.createAdminClient)();
    return next({
        ctx: {
            // infers the `session` as non-nullable
            user: ctx.user,
