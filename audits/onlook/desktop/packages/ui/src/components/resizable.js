...
const ResizablePanel = ({ children, side = 'left', defaultWidth = 240, minWidth = 200, maxWidth = 600, forceWidth, className, }) => {
    ...
    return (<div style={{ width: `${width}px` }} className={(0, utils_1.cn)('h-full relative', isAnimating && 'transition-[width] duration-300 ease-in-out', side === 'left' ? 'left-0' : 'right-0', className)}>
            <div className="h-full">{children}</div>
            <div className={(0, utils_1.cn)('absolute top-0 h-full w-1 cursor-col-resize transition-all', side === 'left' ? 'right-0' : 'left-0')} onMouseDown={handleMouseDown}/>
        </div>);
};
