...
const AccordionTrigger = React.forwardRef(({ className, children, ...props }, ref) => (<AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger ref={ref} className={(0, utils_1.cn)('flex flex-1 items-center justify-between py-4 text-sm font-medium transition-all [&[data-state=open]>svg]:rotate-180', className)} {...props}>
            {children}
            <react_icons_1.ChevronDownIcon className="h-4 w-4 mb-0.5 shrink-0 text-muted-foreground transition-transform duration-200"/>
