...
function extractFontParts(fileName) {
    ...
    if (weight) {
        let match = constants_1.FONT_VARIANTS.find((variant) => variant.name.toLowerCase() === weight.toLowerCase());
        if (!match && /^\d+$/.test(weight)) {
            match = constants_1.FONT_VARIANTS.find((variant) => variant.value === weight);
        }
        if (!match) {
            ...
            match = constants_1.FONT_VARIANTS.find((variant) => {
    ...
    if (!style) {
        style = constants_1.DEFAULT_FONT_STYLE;
    }
    if (!weight) {
        weight = constants_1.DEFAULT_FONT_WEIGHT;
    }
