...
const LOCALES_DIR = path_1.default.join(import.meta.dir, '../src/locales');
// Helper function to get all translation files
async function getTranslationFiles() {
    const locales = await (0, promises_1.readdir)(LOCALES_DIR);
    const translationFiles = [];
    for (const locale of locales) {
        const localePath = path_1.default.join(LOCALES_DIR, locale);
        const stats = await Bun.file(localePath).stat();
        if (stats.isDirectory()) {
            const translationPath = path_1.default.join(localePath, 'translation.json');
            translationFiles.push({ locale, path: translationPath });
        }
    }
...
(0, bun_test_1.describe)('Localization Files', () => {
    (0, bun_test_1.test)('all locales should have translation.json file', async () => {
        const translationFiles = await getTranslationFiles();
        (0, bun_test_1.expect)(translationFiles.length).toBeGreaterThan(0);
        // Log the found locales for debugging
        console.log('Found locales:', translationFiles.map((f) => f.locale).join(', '));
    });
    (0, bun_test_1.test)('all translation files should have the same keys', async () => {
        const translationFiles = await getTranslationFiles();
        (0, bun_test_1.expect)(translationFiles.length).toBeGreaterThan(0);
        // Use English as the reference
        const referenceLocale = translationFiles.find((f) => f.locale === 'en');
        (0, bun_test_1.expect)(referenceLocale).toBeDefined();
        if (!referenceLocale) {
        ...
        for (const file of translationFiles) {
            ...
            (0, bun_test_1.expect)(missingKeys.length).toBe(0);
            if (missingKeys.length > 0) {
                console.error(`Locale ${file.locale} is missing ${missingKeys.length} keys: ${missingKeys.slice(0, 5).join(', ')}${missingKeys.length > 5 ? '...' : ''}`);
            }
            ...
            (0, bun_test_1.expect)(extraKeys.length).toBe(0);
            if (extraKeys.length > 0) {
                console.error(`Locale ${file.locale} has ${extraKeys.length} extra keys: ${extraKeys.slice(0, 5).join(', ')}${extraKeys.length > 5 ? '...' : ''}`);
            }
        }
    });
    (0, bun_test_1.test)('all translation files should have the same structure (value types)', async () => {
        const translationFiles = await getTranslationFiles();
        // Use English as the reference
        const referenceLocale = translationFiles.find((f) => f.locale === 'en');
        (0, bun_test_1.expect)(referenceLocale).toBeDefined();
        ...
        for (const file of translationFiles) {
            ...
            (0, bun_test_1.expect)(typeMismatches.length).toBe(0);
            if (typeMismatches.length > 0) {
                console.error(`Locale ${file.locale} has ${typeMismatches.length} type mismatches: ${typeMismatches
                    .slice(0, 3)
    ...
    (0, bun_test_1.test)('all translation files should have valid interpolation placeholders', async () => {
        const translationFiles = await getTranslationFiles();
        // Use English as the reference
        const referenceLocale = translationFiles.find((f) => f.locale === 'en');
        (0, bun_test_1.expect)(referenceLocale).toBeDefined();
        ...
        const keysWithPlaceholders = new Map();
        for (const key of referenceKeys) {
        ...
        for (const file of translationFiles) {
            ...
            if (placeholderMismatches.length > 0) {
                console.error(`Locale ${file.locale} has placeholder mismatches:`, placeholderMismatches);
            }
            (0, bun_test_1.expect)(placeholderMismatches.length).toBe(0);
            if (placeholderMismatches.length > 0) {
                console.error(`Locale ${file.locale} has ${placeholderMismatches.length} placeholder mismatches: ${placeholderMismatches
                    .slice(0, 3)
                    .map((m) => `${m.key} (missing: ${m.missing.join(', ')}, extra: ${m.extra.join(', ')})`)
