 ...
 */
function findFontClass(className) {
    const fontClass = className
        .split(' ')
        .find((c) => c.startsWith('font-') && !c.match(types_1.FONT_WEIGHT_REGEX));
...
function filterFontClasses(className) {
    return className.split(' ').filter((c) => !c.startsWith('font-') || c.match(types_1.FONT_WEIGHT_REGEX));
}
/**
 * Creates a template literal expression that combines a font variable with existing classes
 */
function createTemplateLiteralWithFont(fontVarExpr, existingValue) {
    const quasis = [
 ...
 */
function createStringLiteralWithFont(fontClassName, existingClasses) {
            ...
            try {
                ...
                try {
                    if (options.fontIds && options.fontIds.length > 0) {
                        ...
                        combinedStatic = combinedStatic.replace(/font-\w+\b/g, (match) => types_1.FONT_WEIGHT_REGEX.test(match) ? match : '');
                    }
                }
                catch (regexError) {
                ...
                if (expressionsToKeep.length === 0) {
                    ...
                    classNameAttr.value = t.stringLiteral(cleanedParts.join('').replace(/\s+/g, ' ').trim());
                }
                else {
                    ...
                    for (let i = 0; i <= expressionsToKeep.length; i++) {
                        ...
                        newQuasis.push(t.templateElement({ raw, cooked: raw }, i === expressionsToKeep.length));
                    }
