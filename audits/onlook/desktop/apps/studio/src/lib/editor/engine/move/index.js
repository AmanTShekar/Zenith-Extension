...
class MoveManager {
    ...
    async start(el, position, webview) {
        ...
        if (el.styles?.computed?.position === 'absolute') {
            ...
            this.originalIndex = await webview.executeJavaScript(`window.api?.startDrag('${el.domId}')`);
        }
    ...
    async drag(e, getRelativeMousePositionToWebview) {
        ...
        if (Math.max(Math.abs(dx), Math.abs(dy)) > this.MIN_DRAG_DISTANCE) {
            ...
            webview.executeJavaScript(`window.api?.drag('${this.dragTarget.domId}', ${dx}, ${dy}, ${x}, ${y})`);
        }
    }
    async handleDragAbsolute(dragOrigin, dragTarget, x, y) {
        const initialOffset = {
        ...
        const offsetParent = await webview.executeJavaScript(`window.api?.getOffsetParent('${dragTarget.domId}')`);
        if (!offsetParent) {
    ...
    async end(e) {
        ...
        const res = await webview.executeJavaScript(`window.api?.endDrag('${this.dragTarget.domId}')`);
        if (res) {
            const { child, parent, newIndex } = res;
            if (newIndex !== this.originalIndex) {
                const moveAction = this.createMoveAction(webview.id, child, parent, newIndex, this.originalIndex);
    ...
    async shiftElement(element, direction) {
        ...
        const currentIndex = await webview.executeJavaScript(`window.api?.getElementIndex('${element.domId}')`);
        if (currentIndex === -1) {
            return;
        }
        ...
        const childrenCount = await webview.executeJavaScript(`window.api?.getChildrenCount('${parent.domId}')`);
        ...
        const moveAction = this.createMoveAction(webview.id, element, parent, newIndex, currentIndex);
        this.editorEngine.action.run(moveAction);
    }
    createMoveAction(webviewId, child, parent, newIndex, originalIndex) {
        return {
            type: 'move-element',
            location: {
                type: 'index',
