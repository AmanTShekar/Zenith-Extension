"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.WebviewEventHandler = void 0;
const constants_1 = require("@onlook/models/constants");
const lodash_1 = require("lodash");
const models_1 = require("../models");
class WebviewEventHandler {
    editorEngine;
    eventCallbacks;
    debouncedHandlers = [];
    constructor(editorEngine) {
        this.editorEngine = editorEngine;
        this.handleIpcMessage = this.handleIpcMessage.bind(this);
        this.handleConsoleMessage = this.handleConsoleMessage.bind(this);
        this.eventCallbacks = {
            [constants_1.WebviewChannels.WINDOW_RESIZED]: this.handleWindowResized(),
            [constants_1.WebviewChannels.WINDOW_MUTATED]: this.handleWindowMutated(),
            [constants_1.WebviewChannels.STYLE_UPDATED]: this.handleStyleUpdated(),
            [constants_1.WebviewChannels.ELEMENT_INSERTED]: this.handleElementInserted(),
            [constants_1.WebviewChannels.ELEMENT_REMOVED]: this.handleElementRemoved(),
            [constants_1.WebviewChannels.ELEMENT_MOVED]: this.handleElementMoved(),
            [constants_1.WebviewChannels.ELEMENT_GROUPED]: this.handleElementGrouped(),
            [constants_1.WebviewChannels.ELEMENT_UNGROUPED]: this.handleElementUngrouped(),
            [constants_1.WebviewChannels.ELEMENT_TEXT_EDITED]: this.handleElementTextEdited(),
            [constants_1.WebviewChannels.DOM_PROCESSED]: this.handleDomProcessed(),
            [constants_1.WebviewChannels.GET_WEBVIEW_ID]: this.handleGetWebviewId(),
        };
    }
    handleDomProcessed() {
        return async (e) => {
            const webview = e.target;
            if (!e.args || e.args.length === 0) {
                console.error('No args found for dom ready event');
                return;
            }
            const { layerMap, rootNode } = e.args[0];
            const processedLayerMap = new Map(Object.entries(layerMap));
            const body = await this.editorEngine.ast.getBodyFromWebview(webview);
            this.editorEngine.ast.setMapRoot(webview.id, body, rootNode, processedLayerMap);
        };
    }
    handleWindowResized() {
        return (e) => {
            const webview = e.target;
            this.editorEngine.elements.refreshSelectedElements(webview);
        };
    }
    handleWindowMutated() {
        const handler = async (e) => {
            const webview = e.target;
            if (!e.args || e.args.length === 0) {
                console.error('No args found for window mutated event');
                return;
            }
            const { added, removed } = e.args[0];
            await this.editorEngine.ast.refreshAstDoc(webview);
            const newMap = new Map([...Object.entries(added), ...Object.entries(removed)]);
            this.editorEngine.ast.updateMap(webview.id, newMap, null);
        };
        const debouncedHandler = (0, lodash_1.debounce)(handler, 1000, { leading: true, trailing: true });
        this.debouncedHandlers.push(() => debouncedHandler.cancel());
        return debouncedHandler;
    }
    handleElementInserted() {
        return async (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for insert element event');
                return;
            }
            const { domEl, layerMap, editText } = e.args[0];
            const webview = e.target;
            if (!webview) {
                console.error('No webview found for insert element event');
                return;
            }
            await this.refreshAndClickMutatedElement(domEl, layerMap, webview);
            if (editText) {
                this.editorEngine.text.start(domEl, webview);
            }
        };
    }
    handleElementRemoved() {
        return async (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for move element event');
                return;
            }
            const { parentDomEl, layerMap } = e.args[0];
            const webview = e.target;
            this.refreshAndClickMutatedElement(parentDomEl, layerMap, webview);
        };
    }
    handleElementTextEdited() {
        return async (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for move element event');
                return;
            }
            const { domEl, layerMap } = e.args[0];
            const webview = e.target;
            this.refreshAndClickMutatedElement(domEl, layerMap, webview);
        };
    }
    handleElementMoved() {
        return async (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for move element event');
                return;
            }
            const { domEl, layerMap } = e.args[0];
            const webview = e.target;
            this.refreshAndClickMutatedElement(domEl, layerMap, webview);
        };
    }
    handleElementGrouped() {
        return async (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for move element event');
                return;
            }
            const { domEl, layerMap } = e.args[0];
            const webview = e.target;
            this.refreshAndClickMutatedElement(domEl, layerMap, webview);
        };
    }
    handleElementUngrouped() {
        return async (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for move element event');
                return;
            }
            const { parentEl, layerMap } = e.args[0];
            const webview = e.target;
            this.refreshAndClickMutatedElement(parentEl, layerMap, webview);
        };
    }
    async refreshAndClickMutatedElement(domEl, newMap, webview) {
        this.editorEngine.mode = models_1.EditorMode.DESIGN;
        await this.editorEngine.ast.refreshAstDoc(webview);
        this.editorEngine.elements.click([domEl], webview);
        this.editorEngine.ast.updateMap(webview.id, newMap, domEl.domId);
    }
    handleStyleUpdated() {
        const handler = (e) => {
            if (!e.args || e.args.length === 0) {
                console.error('No args found for style updated event');
                return;
            }
            const { domEl } = e.args[0];
            const webview = e.target;
            this.editorEngine.elements.click([domEl], webview);
        };
        const debouncedHandler = (0, lodash_1.debounce)(handler, 100, { leading: true, trailing: true });
        this.debouncedHandlers.push(() => debouncedHandler.cancel());
        return debouncedHandler;
    }
    handleGetWebviewId() {
        return async (e) => {
            const webview = e.target;
            await webview.executeJavaScript(`window.api.setWebviewId('${webview.id}')`);
            await webview.executeJavaScript(`window.api.processDom()`);
        };
    }
    handleIpcMessage(e) {
        const eventHandler = this.eventCallbacks[e.channel];
        if (!eventHandler) {
            console.error(`No event handler found for ${e.channel}`);
            return;
        }
        eventHandler(e);
    }
    handleConsoleMessage(e) {
        console.log(`%c ${e.message}`, 'background: #000; color: #AAFF00');
    }
    dispose() {
        // Clean up debounced handlers
        this.debouncedHandlers.forEach((cancel) => cancel());
        this.debouncedHandlers = [];
        // Clean up event callbacks
        Object.keys(this.eventCallbacks).forEach((channel) => {
            // Remove the callback from any event listeners
            delete this.eventCallbacks[channel];
        });
        // Clear references
        this.eventCallbacks = {};
        this.editorEngine = null;
    }
}
exports.WebviewEventHandler = WebviewEventHandler;
//# sourceMappingURL=eventHandler.js.map