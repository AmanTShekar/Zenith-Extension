...
class GroupManager {
    ...
    getGroupParentId(elements, log = true) {
    ...
    async getGroupAction(webviewId, parentDomId, selectedEls) {
        ...
        const container = {
            domId: (0, utils_1.createDomId)(),
            oid: (0, utils_1.createOid)(),
            tagName: 'div',
            attributes: {},
        };
        return {
    ...
    async getUngroupAction(selectedEl) {
        ...
        const actionContainer = await webview.executeJavaScript(`window.api?.getActionElementByDomId('${selectedEl.domId}', true)`);
        if (!actionContainer) {
        ...
        const targets = actionContainer.children.map((child) => {
            return {
