...
exports.OverlayChat = (0, mobx_react_lite_1.observer)(({ selectedEl, elementId }) => {
    const editorEngine = (0, Context_1.useEditorEngine)();
    const userManager = (0, Context_1.useUserManager)();
    const isPreviewMode = editorEngine.mode === models_1.EditorMode.PREVIEW;
    const [inputState, setInputState] = (0, react_1.useState)(DEFAULT_INPUT_STATE);
    const [isComposing, setIsComposing] = (0, react_1.useState)(false);
    const textareaRef = (0, react_1.useRef)(null);
    const prevChatPositionRef = (0, react_1.useRef)(null);
    const { t } = (0, react_i18next_1.useTranslation)();
    const shouldHideButton = !selectedEl ||
        isPreviewMode ||
    ...
    return (<div style={containerStyle} onClick={(e) => e.stopPropagation()} className={animationClass} data-element-id={elementId}>
                <div className={(0, utils_1.cn)('rounded-xl backdrop-blur-lg transition-all duration-300', 'shadow-xl shadow-background-secondary/50', inputState.isVisible
        ...
        // Chat Button
        <button onClick={() => setInputState((prev) => ({ ...prev, isVisible: true }))} className="rounded-lg hover:text-foreground-primary transition-colors px-2.5 py-1.5 flex flex-row items-center gap-2 w-full">
                            <index_1.Icons.Sparkles className="w-4 h-4"/>
        ...
        <div className="flex flex-row items-top gap-1 w-full min-w-[280px] relative">
                ...
                isVisible: false,
                value: '',
            }))} className={(0, utils_1.cn)('group h-6 w-6 absolute left-1 top-1 z-10 border-none shadow-none bg-transparent hover:bg-transparent', 'transition-all duration-200', inputState.value.trim().length >= DIMENSIONS.minCharsToSubmit
                ...
                : 'opacity-100 translate-x-0 scale-100 pointer-events-auto')} disabled={inputState.isSubmitting}>
                                <index_1.Icons.CrossS className="h-4 w-4 text-foreground-secondary group-hover:text-foreground transition-colors"/>
                            </button_1.Button>
                            <textarea_1.Textarea aria-label="Chat message input" ref={textareaRef} className={(0, utils_1.cn)('w-full text-xs break-words p-1.5 focus-visible:ring-0 resize-none shadow-none border-[0.5px] rounded-lg', 'transition-all duration-150 ease-in-out', 'pr-10 backdrop-blur-lg', inputState.value.trim().length >= DIMENSIONS.minCharsToSubmit
                ? 'pl-2'
                : 'pl-8', 'bg-background-secondary/75 text-foreground-primary border-background-secondary/75', 'max-h-[80px] caret-[#FA003C]', 'selection:bg-[#FA003C]/30 selection:text-[#FA003C]')} value={inputState.value} onChange={(e) => {
                ...
                setIsComposing(false);
                            ...
                            {inputState.value.trim().length >= DIMENSIONS.minCharsToSubmit && (<button_1.Button size="icon" variant="secondary" onClick={handleSubmit} className={(0, utils_1.cn)('absolute right-0.5 bottom-0.5 h-7 w-7', 'bg-foreground-primary text-white hover:bg-foreground-hover')} disabled={inputState.isSubmitting}>
                                    <index_1.Icons.ArrowRight className="h-4 w-4 text-background"/>
                                </button_1.Button>)}
                        </div>)}
                </div>
            </div>);
