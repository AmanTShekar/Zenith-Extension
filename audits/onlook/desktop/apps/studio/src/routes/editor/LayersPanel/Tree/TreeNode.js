...
const NodeIcon_1 = __importDefault(require("./NodeIcon"));
...
const TreeNode = (0, mobx_react_lite_1.observer)(({ node, style, dragHandle, }) => {
    ...
    const handleHoverNode = (0, react_2.useCallback)((e) => {
        if (hovered) {
            return;
        }
        sendMouseEvent(e, node.data, editor_1.MouseAction.MOVE);
    }, [hovered, node.data]);
    const handleSelectNode = (0, react_2.useCallback)((e) => {
        if (selected) {
            return;
        }
        node.select();
        sendMouseEvent(e, node.data, editor_1.MouseAction.MOUSE_DOWN);
    }, [selected, node]);
    const nodeClassName = (0, react_2.useMemo)(() => (0, tailwind_merge_1.twMerge)((0, utils_1.cn)('flex flex-row items-center h-6 cursor-pointer w-full pr-1', {
    ...
    async function sendMouseEvent(e, node, action) {
        const webview = editorEngine.webviews.getWebview(node.webviewId);
        if (!webview) {
            console.error('Failed to get webview');
            return;
        }
        const el = await webview.executeJavaScript(`window.api?.getDomElementByDomId('${node.domId}', ${action === editor_1.MouseAction.MOUSE_DOWN})`);
        if (!el) {
    ...
    return (<tooltip_1.Tooltip>
                ...
                : selected
                    ...
                    : 'text-purple-500 dark:text-purple-300')}/>) : (<NodeIcon_1.default iconClass={(0, utils_1.cn)('w-3 h-3 ml-1 mr-2 flex-none', {
...
exports.default = (0, react_2.memo)(TreeNode);
