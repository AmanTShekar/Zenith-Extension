...
exports.canCompressFile = canCompressFile;
...
async function compressImage(file, options = {}, onProgress) {
                ...
                if (ctx) {
                    ...
                    const tryCompression = () => {
                        canvas.toBlob((blob) => {
                            ...
                            const compressedFile = new File([blob], file.name.replace(/\.[^/.]+$/, '') + getExtensionForFormat(opts.format), { type: opts.format });
                            onProgress?.(100);
                            resolve({
                                file: compressedFile,
...
async function compressMultipleImages(files, options = {}, onProgress) {
    const results = [];
