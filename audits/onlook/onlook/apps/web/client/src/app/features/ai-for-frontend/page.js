...
exports.default = AiForFrontendPage;
...
const ai_frontend_hero_1 = require("../../_components/hero/ai-frontend-hero");
    ...
    return (<create_1.CreateManagerProvider>
            <website_layout_1.WebsiteLayout showFooter={true}>
                ...
                <div className="flex h-screen w-screen items-center justify-center" id="hero">
                    <ai_frontend_hero_1.AiFrontendHero />
                </div>
                ...
                <section className="w-full bg-black py-32">
                    <div className="mx-auto max-w-6xl px-8">
                        ...
                        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
                            {[
            {
                icon: icons_1.Icons.CrossCircled,
            ...
            },
            {
        ...
        ].map((category, categoryIndex) => (<react_1.motion.div key={category.title} className="border-foreground-primary/10 rounded-lg border p-6" {...getBlurAnimationProps(0.2 + categoryIndex * 0.1)}>
                                    <h3 className="text-foreground-secondary mb-4 text-sm font-medium uppercase tracking-wider">{category.title}</h3>
                                    <ul className="flex flex-col gap-2">
                                        {category.items.map((item, itemIndex) => (<react_1.motion.li key={item} className="text-foreground-primary text-base" initial={{ opacity: 0, y: 5 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{
                    duration: 0.3,
                ...
                {/* FAQ Section */}
                <faq_section_1.FAQSection faqs={aiFrontendFaqs} title="Frequently asked questions"/>

                {/* CTA Section */}
                <cta_section_1.CTASection ctaText={`Start building with AI\nthat knows your stack`} buttonText="Book a Demo" href={constants_1.ExternalRoutes.BOOK_DEMO}/>
