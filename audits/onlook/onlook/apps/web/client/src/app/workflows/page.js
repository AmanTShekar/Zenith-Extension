...
function WorkflowsPage() {
    return (<create_1.CreateManagerProvider>
            <website_layout_1.WebsiteLayout showFooter={true}>
                ...
                <section className="w-full bg-black py-32">
                    <div className="mx-auto max-w-6xl px-8">
                        <div className="grid gap-8 md:grid-cols-3 max-w-5xl mx-auto">
                            {workflows.map((workflow, index) => (<react_1.motion.div key={workflow.title} {...getBlurAnimationProps(0.1 + index * 0.1)}>
                                    {workflow.available ? (<link_1.default href={workflow.href} className="group border-foreground-primary/10 hover:border-foreground-primary/30 flex h-full flex-col gap-4 rounded-lg border bg-black p-8 transition-all duration-300">
                                            {workflow.logo ? (<img src={workflow.logo} alt={workflow.title} className="h-10 w-10"/>) : workflow.icon ? (<workflow.icon className="text-foreground-secondary group-hover:text-foreground-primary h-10 w-10 transition-colors"/>) : null}
                                        ...
                                        </link_1.default>) : (<div className="border-foreground-primary/10 flex h-full flex-col gap-4 rounded-lg border bg-black p-8">
                                            {workflow.logo ? (<img src={workflow.logo} alt={workflow.title} className="h-10 w-10 opacity-50"/>) : workflow.icon ? (<workflow.icon className="text-foreground-tertiary h-10 w-10 opacity-50"/>) : null}
                ...
                {/* CTA Section */}
                <cta_section_1.CTASection ctaText={`Ready to add Onlook\nto your workflow?`} buttonText="Book a Demo" href={constants_1.ExternalRoutes.BOOK_DEMO}/>
