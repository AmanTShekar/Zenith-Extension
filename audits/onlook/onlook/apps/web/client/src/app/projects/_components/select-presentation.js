...
exports.SelectProjectPresentation = void 0;
...
const template_modal_presentation_1 = require("./templates/template-modal-presentation");
const highlight_text_1 = require("./select/highlight-text");
const masonry_layout_1 = require("./select/masonry-layout");
const project_card_presentation_1 = require("./select/project-card-presentation");
const square_project_card_presentation_1 = require("./select/square-project-card-presentation");
/**
...
const SelectProjectPresentation = ({ allProjects = [], isLoading = false, externalSearchQuery = '', isCreatingProject = false, onCreateBlank, starredTemplateIds = new Set(), onToggleStar, onUnmarkTemplate, onRefetch, user, onProjectClick, onRenameProject, onCloneProject, onToggleTemplate, onDeleteProject, onUseTemplate, onPreviewTemplate, onEditTemplate, }) => {
    // Search and filters
    ...
    // Templates
    const projects = (0, react_1.useMemo)(() => allProjects.filter((project) => !project.metadata.tags.includes('template')), [allProjects]);
    const templateProjects = (0, react_1.useMemo)(() => allProjects.filter((project) => project.metadata.tags.includes('template')), [allProjects]);
    ...
    if (projects.length === 0 && !debouncedSearchQuery) {
        return (<div className="flex h-full w-full flex-col items-center justify-center gap-4">
                ...
                <div className="flex justify-center">
                    <button_1.Button onClick={onCreateBlank} disabled={isCreatingProject} variant="default">
    ...
    return (<div className="relative flex h-full w-full flex-col overflow-x-visible px-6 py-8" style={{
            ...
            <react_2.motion.div key="create-tile" className="w-72 flex-shrink-0" initial={{ opacity: 0, y: 20, filter: 'blur(10px)' }} animate={{
                    ...
                    transition: { duration: 0.2 },
                                        ...
                                        <button onClick={onCreateBlank} disabled={isCreatingProject} className="border-border bg-secondary/40 hover:bg-secondary relative flex aspect-[4/2.8] w-full items-center justify-center rounded-lg border transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            ...filteredAndSortedProjects.map((project, index) => (<react_2.motion.div key={project.id} className="w-72 flex-shrink-0" initial={{ opacity: 0, y: 20, filter: 'blur(10px)' }} animate={{
                    ...
                    transition: { duration: 0.2 },
                                            ...
                                            <square_project_card_presentation_1.SquareProjectCardPresentation project={project} imageUrl={getImageUrl(project)} searchQuery={debouncedSearchQuery} HighlightText={highlight_text_1.HighlightText} onClick={onProjectClick}/>
                                        </react_2.motion.div>))
        ])}
                ...
                {shouldShowTemplate && (<templates_1.Templates templateProjects={templateProjects} searchQuery={debouncedSearchQuery} onTemplateClick={handleTemplateClick} onToggleStar={handleToggleStar} starredTemplates={starredTemplateIds}/>)}
                ...
                ease: [0.25, 0.46, 0.45, 0.94],
                    ...
                    {layoutMode === 'masonry' ? (<masonry_layout_1.MasonryLayout items={filesProjects} spacing={spacing} renderItem={(project, aspectRatio) => (<project_card_presentation_1.ProjectCardPresentation key={`files-${project.id}`} project={project} imageUrl={getImageUrl(project)} aspectRatio={aspectRatio} searchQuery={debouncedSearchQuery} HighlightText={highlight_text_1.HighlightText} onEdit={onProjectClick} onRename={onRenameProject} onClone={onCloneProject} onToggleTemplate={onToggleTemplate} onDelete={onDeleteProject} isTemplate={project.metadata.tags.includes('template')}/>)}/>) : (<react_2.motion.div layout className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {filesProjects.map((project) => (<project_card_presentation_1.ProjectCardPresentation key={`files-${project.id}`} project={project} imageUrl={getImageUrl(project)} aspectRatio="aspect-[4/2.6]" searchQuery={debouncedSearchQuery} HighlightText={highlight_text_1.HighlightText} onEdit={onProjectClick} onRename={onRenameProject} onClone={onCloneProject} onToggleTemplate={onToggleTemplate} onDelete={onDeleteProject} isTemplate={project.metadata.tags.includes('template')}/>))}
                        </react_2.motion.div>)}
                </div>
            </div>

            {selectedTemplate && shouldShowTemplate && (<template_modal_presentation_1.TemplateModalPresentation isOpen={isTemplateModalOpen} onClose={handleCloseTemplateModal} title={selectedTemplate.name} description={selectedTemplate.metadata?.description || 'No description available'} image={getImageUrl(selectedTemplate)} isNew={false} isStarred={selectedTemplate ? starredTemplateIds.has(selectedTemplate.id) : false} onToggleStar={() => selectedTemplate && handleToggleStar(selectedTemplate.id)} templateProject={selectedTemplate} onUnmarkTemplate={handleUnmarkTemplate} onUseTemplate={() => selectedTemplate && onUseTemplate?.(selectedTemplate.id)} onPreviewTemplate={() => selectedTemplate && onPreviewTemplate?.(selectedTemplate.id)} onEditTemplate={() => selectedTemplate && onEditTemplate?.(selectedTemplate.id)}/>)}
        </div>);
};
exports.SelectProjectPresentation = SelectProjectPresentation;
//# sourceMappingURL=select-presentation.js.map