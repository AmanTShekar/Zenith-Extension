...
exports.LayersBlock = LayersBlock;
...
function MockLayersTab() {
    ...
    const [selectedId, setSelectedId] = react_1.default.useState('3');
    return (<div className="w-72 shadow-lg p-2 overflow-hidden max-h-96 flex flex-col gap-1">
            <div className="flex flex-col gap-0.5">
                {mockLayers.map((layer) => {
            const isComponent = layer.tagName === 'COMPONENT';
            const isSelected = selectedId === layer.id;
            const isHovered = hoveredId === layer.id;
            return (<div key={layer.id} className={(0, utils_1.cn)('flex items-center h-5.5 px-2 cursor-pointer transition-colors select-none text-xs', 
                // Component styling
    ...
    return (<div className="flex flex-col gap-6">
                ...
                <div className="w-90 h-80 absolute top-20 left-1/4 z-10 rounded-lg overflow-hidden">
                    ...
                    <div className="w-full h-full absolute top-0 left-0 flex flex-col items-center justify-start">
                        <div className="w-full h-6 bg-gray-800/80 flex flex-row gap-4 justify-between items-center px-2 absolute top-1.5 left-0">
                            <illustrations_1.Illustrations.LayersLogo className='h-2 ml-34'/>
                            <illustrations_1.Illustrations.LayersReserve className='h-1'/>
                        </div>
                        <div className='w-full h-12 absolute top-10 left-5 flex flex-row gap-4 items-center justify-center'>
                            <illustrations_1.Illustrations.LayersEndorphins className='w-56'/>
                            <illustrations_1.Illustrations.LayersInMotion className='w-48 top-12 absolute'/>
                        </div>
                        <div className='absolute bottom-3 w-fit h-fit flex flex-col items-center justify-start bg-[#872D2D] p-2 hover:bg-red-600 transition-colors duration-300 cursor-pointer'>
                            <illustrations_1.Illustrations.LayersOrderNow className='w-18'/>
                        </div>
                    </div>
            ...
            <div className="flex flex-row items-start gap-8 w-full">
                ...
                <div className="flex flex-col items-start w-1/2">
                    <div className="mb-2"><icons_1.Icons.Layers className="w-6 h-6 text-foreground-primary"/></div>
