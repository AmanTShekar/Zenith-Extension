...
const generateFAQJsonLd = () => {
    const allFaqs = faqSections.flatMap(section => section.faqs.map(faq => ({
        '@type': 'Question',
        name: faq.question,
        acceptedAnswer: {
            '@type': 'Answer',
...
function FAQPage() {
    const [currentSection, setCurrentSection] = (0, react_1.useState)(faqSections[0]?.anchor || '');
    const sectionRefs = (0, react_1.useRef)([]);
    const faqContainerRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        const handleScroll = () => {
            const offset = 120;
            let activeIdx = 0;
            ...
            <div className="w-full max-w-6xl mx-auto py-32 px-4 md:px-8">
                    ...
                    <nav className="hidden lg:block w-48 flex-shrink-0 self-start sticky top-32">
                        <div>
                            ...
                            <ul className="flex flex-col gap-2">
                                {faqSections.map((section) => (<li key={section.anchor}>
                ...
                : 'text-foreground-tertiary hover:text-foreground-secondary'}`}>
                    ...
                    <section className="flex-1 max-w-[800px]">
                        {faqSections.map((section, i) => (<div key={section.anchor} id={section.anchor} className="mb-16 scroll-mt-24" ref={el => { sectionRefs.current[i] = el; }}>
                                <h2 className="text-foreground-primary text-2xl font-medium mb-6">{section.title}</h2>
                                <faq_dropdown_1.FAQDropdown faqs={section.faqs}/>
                            </div>))}
                    </section>
                </div>
            </div>

            <cta_section_1.CTASection ctaText={`Still have questions?`} buttonText="Book a Demo" href={constants_1.ExternalRoutes.BOOK_DEMO}/>
