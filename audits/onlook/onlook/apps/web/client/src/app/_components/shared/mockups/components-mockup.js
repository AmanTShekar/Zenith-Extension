...
exports.ComponentsMockup = ComponentsMockup;
const icons_1 = require("@onlook/ui/icons");
const react_1 = __importDefault(require("react"));
function ComponentsMockup() {
    return (<div className="flex flex-row gap-8 relative min-h-[400px] overflow-hidden bg-background-onlook/80 rounded-lg">
            ...
            { label: 'Weather', selected: false },
        ].map((item, idx) => (<div key={item.label} className="flex flex-col items-center w-full">
                            <div className={`w-24 h-24 rounded-xs mb-1.5 flex items-center justify-center bg-background-secondary transition-all ` +
                (item.selected
                    ? 'outline outline-1 outline-purple-400 outline-offset-2'
                    : '')}>
                                {/* Custom component previews */}
                                {item.label === 'Calendar' && (<div className="w-16 h-fit bg-black rounded-[4px] p-1.5 select-none" style={{ fontSize: '3px' }}>
                                        {/* Mini calendar header */}
                                        <div className="flex items-center justify-between mb-1 mx-0.5">
                                            <icons_1.Icons.ArrowLeft className="w-1 h-1 text-foreground-primary"/>
                                            ...
                                            <icons_1.Icons.ArrowRight className="w-1 h-1 text-foreground-primary"/>
                                        </div>
                                        {/* Mini calendar grid */}
                    ...
                    return Array.from({ length: 35 }, (_, i) => {
                        ...
                        return (<div key={i} className={`py-[0.5px] rounded-[4px] text-[3px] ${isToday ? 'bg-white text-black font-bold' : 'text-zinc-300'}`}>
                                                            {day}
                                                        </div>);
                    });
                })()}
                                ...
                                {item.label === 'Card' && (<div className="w-18 h-18 bg-black rounded-[4px] p-1.5 flex flex-col gap-[1.5px] select-none" style={{ fontSize: '3.5px' }}>
                                        {/* Header */}
                                ...
                                {item.label === 'Carousel' && (<div className="w-20 h-20 bg-transparent flex items-center justify-center select-none">
                                        {/* Left arrow button */}
                                ...
                                {item.label === 'Chart' && (<div className="w-18 h-16 bg-black rounded-[4px] p-0.5 flex flex-col select-none overflow-hidden relative">
                                        {/* Header */}
                    ...
                    13, 15, 9, 17, 8, 12, 14, 6, 18, 11
                ].map((h, i) => (<div key={i} className="bg-teal-300 rounded-[1px]" style={{ width: '1.2px', height: `${h * 1.6}px` }}/>))}
                                            </div>
                                        </div>
                                    </div>)}
                                {item.label === 'Table' && (<div className="w-16 h-16 bg-black rounded-[4px] p-0.5 select-none">
                                ...
                                {item.label === 'Weather' && (<div className="w-16 h-16 bg-black rounded-[4px] p-1 flex flex-col items-center select-none relative overflow-hidden">
                                        {/* Sun icon */}
                                        <div className="flex items-center justify-center w-full mt-2">
                                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                                                <circle cx="8" cy="8" r="4" fill="#fbbf24"/>
                                                {[...Array(10)].map((_, i) => (<rect key={i} x={8 - 0.5} y={1} width={1} height={3} fill="#fbbf24" transform={`rotate(${i * 45} 8 8)`}/>))}
                                            </svg>
            ...
            <div className="absolute md:right-1/30 right-1/10 top-30 z-10">
                <div className="rounded-xl border-1 border-purple-400 bg-black p-4 min-w-[240px]" style={{ fontSize: '0.6rem' }}>
                    ...
                    <icons_1.Icons.ArrowLeft className="w-4 h-4 text-foreground-primary"/>
                        ...
                        <icons_1.Icons.ArrowRight className="w-4 h-4 text-foreground-primary"/>
                    </div>
                    {/* Calendar grid */}
            ...
            return Array.from({ length: 42 }, (_, i) => {
                ...
                if (day < 1 || day > daysInMonth)
                    return <div key={i}></div>;
                return (<div key={i} className={`py-[2px] rounded-full text-xs ` +
...
//# sourceMappingURL=components-mockup.js.map