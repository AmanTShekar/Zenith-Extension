...
const upload_modal_1 = __importDefault(require("./upload-modal"));
const FontPanel = (0, mobx_react_lite_1.observer)(() => {
    ...
    const handleFontUpload = async (fonts) => {
        try {
            const success = await fontManager.uploadFonts(fonts);
            if (success) {
                sonner_1.toast.success('Fonts uploaded successfully');
            }
            else {
                sonner_1.toast.error('Failed to upload fonts');
            }
            return success;
        }
        catch (error) {
    ...
    const performSearch = (0, react_1.useCallback)(async (value) => {
        if (value.length > 0) {
            setIsLoading(true);
            try {
                ...
                sonner_1.toast.error('Failed to search fonts', {
                    description: error instanceof Error ? error.message : 'Unknown error',
                });
            }
            finally {
    ...
    const handleLoadMore = async () => {
        ...
        try {
            ...
            sonner_1.toast.error('Failed to load more fonts', {
                description: error instanceof Error ? error.message : 'Unknown error',
            });
        }
        finally {
    ...
    return (<div className="flex flex-col h-full text-xs text-active flex-grow w-full p-0">
            ...
            <div className="px-4 py-3 border-b border-border">
                <div className="relative">
                    ...
                    <input_1.Input ref={inputRef} type="text" placeholder="Search for a new font..." className="h-9 text-xs pl-7 pr-8" value={searchQuery} onChange={(e) => handleSearch(e.target.value)} onKeyDown={handleKeyDown}/>
                    {searchQuery && (<button className="absolute right-[1px] top-[1px] bottom-[1px] aspect-square hover:bg-background-onlook active:bg-transparent flex items-center justify-center rounded-r-[calc(theme(borderRadius.md)-1px)] group" onClick={() => handleSearch('')}>
                            <icons_1.Icons.CrossS className="h-3 w-3 text-foreground-primary/50 group-hover:text-foreground-primary"/>
                        </button>)}
                    {isLoading && searchQuery && (<div className="absolute right-9 top-1/2 -translate-y-1/2">
                            <icons_1.Icons.LoadingSpinner className="h-4 w-4 animate-spin text-muted-foreground"/>
                        </div>)}
                </div>
            </div>
            ...
            <div className="flex flex-col flex-1 overflow-y-auto">
                ...
                {searchQuery === '' && (<div className="flex flex-col gap-1 pt-6 pb-3 border-b border-border">
                        {/* System Fonts Header */}
                        <div className="px-4">
                            <div className="flex items-center gap-2">
                                ...
                                {fontManager.isScanning && (<icons_1.Icons.LoadingSpinner className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                            </div>
                        </div>
                ...
                <div className="flex flex-col gap-1 pt-6 pb-4">
                    ...
                    <div className="px-4">
                        <div className="flex flex-col divide-y divide-border">
                            {uniqueSiteFonts?.length > 0 ? (uniqueSiteFonts.map((font, index) => (<div key={`${font.family}-${index}`}>
                                        <div className="flex justify-between items-center">
                                            <font_family_1.FontFamily name={font.family} variants={font.weight?.map((weight) => fonts_1.VARIANTS.find((v) => v.value === weight)
                ?.name).filter((v) => v !== undefined) ?? []} showDropdown={false} showAddButton={true} onAddFont={() => fontManager.addFont(font)}/>
                                        </div>
                                    </div>))) : (<div className="flex justify-center items-center h-20 my-2">
                                    <span className="text-sm text-muted-foreground">
                        ...
                        {/* Load More Button */}
                        {fontManager.hasMoreFonts && !searchQuery && (<button_1.Button variant="ghost" className="w-full mt-4 h-9 text-sm text-muted-foreground hover:text-foreground bg-background-secondary hover:bg-background-secondary/70 rounded-lg border border-white/5" onClick={handleLoadMore} disabled={isLoading}>
            ...
            <div className="p-4 border-t border-border mt-auto">
                <button_1.Button variant="ghost" className="w-full h-11 text-sm text-muted-foreground hover:text-foreground bg-background-secondary hover:bg-background-secondary/70 rounded-lg border border-white/5" onClick={handleUploadFont}>
                    Upload a custom font
                </button_1.Button>
            </div>

            <upload_modal_1.default isOpen={isUploadModalOpen} onOpenChange={setIsUploadModalOpen} onUpload={handleFontUpload} isUploading={fontManager.isUploading}/>
        </div>);
});
exports.default = FontPanel;
