...
exports.CodeOneToOneSection = CodeOneToOneSection;
...
function CodeOneToOneSection() {
    const [selectedDiv, setSelectedDiv] = (0, react_1.useState)(null);
    return (<section className="relative w-full flex justify-center bg-transparent min-h-[480px] max-w-6xl mx-auto px-8">
      ...
      <div className="relative w-full max-w-6xl mx-auto flex items-center justify-start min-h-[420px] mt-24" style={{ paddingTop: '7vw' }}>
        ...
        <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-[800px] h-[320px] flex flex-col items-center justify-center z-10">
          <div className="w-full h-full flex flex-col justify-between p-8 gap-6">
            {mockDivs.map((div) => (<div key={div.id} className={`w-full h-14 rounded-lg flex items-center justify-end pr-8 cursor-pointer transition border border-border/10 text-xl font-light ${selectedDiv === div.id ? 'border-blue-500 bg-blue-50' : 'bg-white hover:border-blue-300'}`} onClick={() => setSelectedDiv(div.id)} style={{ boxShadow: selectedDiv === div.id ? '0 0 0 2px #3b82f6' : undefined }}>
        ...
        <div className="absolute right-24 top-1/2 -translate-y-1/2 z-20" style={{ transform: 'translateY(-50%) translateX(20%)' }}>
          <div className="backdrop-blur-xl bg-background/80 border border-border/10 border-[0.5px] rounded-2xl shadow-xl shadow-black/40 p-3 w-[440px] h-[400px] flex flex-col text-left text-small font-mono text-foreground-secondary relative overflow-hidden">
            ...
            return (<div key={idx} className={`transition flex flex-row items-start text-small px-0 rounded ${highlight ? 'bg-blue-900/60 text-blue-200' : ''}`}>
                    <span className="select-none text-small text-foreground-tertiary/50 font-mono w-6 text-left mr-2 pt-0.5" aria-hidden="true">
                      {idx + 1}
                    </span>
...
//# sourceMappingURL=code-one-to-one-section.js.map