...
exports.CodeProviderSync = void 0;
exports.hashContent = hashContent;
const helpers_1 = require("@/components/store/editor/sandbox/helpers");
...
class CodeProviderSync {
    ...
    fileHashes = new Map();
    instanceKey = null;
    constructor(provider, fs, config = { include: [], exclude: [] }) {
     ...
     */
    static getInstance(provider, fs, sandboxId, config = { include: [], exclude: [] }) {
        ...
        if (existing) {
            ...
            const sameConfig = JSON.stringify(existing.sync.config ?? {}) === JSON.stringify(config ?? {});
            if (!sameConfig) {
                console.warn(`[Sync] getInstance(${key}) called with different config; reusing existing instance config`);
            }
            existing.refCount++;
     ...
     */
    static generateKey(sandboxId, fs) {
    ...
    async pullFromSandbox() {
        const sandboxEntries = await this.getAllSandboxFiles('./');
        const sandboxEntriesSet = new Set(sandboxEntries.map((e) => (e.path.startsWith('/') ? e.path : `/${e.path}`)));
        ...
        for (const entry of entriesToDelete) {
            try {
                ...
                console.debug(`[Sync] Failed to delete ${entry.path}:`, error instanceof Error ? error.message : 'Unknown error');
            }
        }
    ...
    async getAllSandboxFiles(dir) {
        ...
        try {
            ...
            console.debug(`[Sync] Error reading directory ${dir}:`, error instanceof Error ? error.message : 'Unknown error');
        }
        return files;
    }
    async pushModifiedFilesToSandbox() {
        ...
        try {
            ...
            await Promise.all(jsxFiles.map(async (filePath) => {
                try {
    ...
    async setupWatching() {
        try {
                ...
                onFileChange: async (event) => {
                    ...
                    if (event.type === 'change' || event.type === 'add') {
                        ...
                        if (event.type === 'change' &&
                            ...
                            const oldPath = (0, helpers_1.normalizePath)(event.paths[0]);
                            const newPath = (0, helpers_1.normalizePath)(event.paths[1]);
                                ...
                                try {
                                    ...
                                    if (await this.fs.exists(oldPath)) {
                                        ...
                                        try {
                                            ...
                                            const { file } = result;
                                            if ((file.type === 'text' || file.type === 'binary') &&
                                                file.content) {
                                            ...
                                            console.error(`[Sync] Error creating ${newPath}:`, error);
                                        }
                                    }
                                }
                                catch (error) {
                            ...
                            for (const path of event.paths) {
                                ...
                                const normalizedPath = (0, helpers_1.normalizePath)(path);
                                ...
                                try {
                                    ...
                                    if (stat.type === 'directory') {
                                        ...
                                        try {
                                            ...
                                            const syncDirectoryContents = async (sandboxPath, localDirPath) => {
                                                try {
                        ...
                        for (const path of event.paths) {
                            ...
                            const normalizedPath = (0, helpers_1.normalizePath)(path);
                            if (!this.shouldSync(normalizedPath)) {
                                console.debug(`[Sync] Skipping sandbox delete for excluded path: ${normalizedPath}`);
                                continue;
                            }
                            try {
                                const localPath = normalizedPath;
                                ...
                                console.debug(`[Sync] Error deleting ${normalizedPath} locally:`, error instanceof Error ? error.message : 'Unknown error');
                            }
                        }
                    }
                },
            ...
            console.error('[Sync] Failed to setup file watching:', error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    async setupLocalWatching() {
        ...
        this.localWatcher = this.fs.watchDirectory('/', async (event) => {
            ...
            try {
                switch (type) {
                    ...
                    case 'delete': {
                        ...
                        try {
                            ...
                            console.debug(`[Sync] Failed to delete ${sandboxPath} from sandbox:`, error instanceof Error ? error.message : 'Unknown error');
                        }
                ...
                console.error(`[Sync] Error pushing local ${type} for ${path} to sandbox:`, error instanceof Error ? error.message : 'Unknown error');
            }
        });
    }
}
exports.CodeProviderSync = CodeProviderSync;
