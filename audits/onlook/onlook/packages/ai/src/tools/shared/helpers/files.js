...
exports.safeRunCommand = safeRunCommand;
exports.isCommandAvailable = isCommandAvailable;
exports.resolvePath = resolvePath;
exports.findFuzzyPath = findFuzzyPath;
async function getFileSystem(branchId, editorEngine) {
...
async function resolveDirectoryPath(inputPath, sandbox) {
    if (!inputPath) {
...
async function safeRunCommand(sandbox, command, fallbackValue) {
    try {
...
async function resolvePath(inputPath, sandbox) {
        ...
        try {
            ...
            if (currentDirResult) {
                const candidates = currentDirResult
                    .map((file) => file.name)
                    .filter((name) => name.includes(targetName) ||
