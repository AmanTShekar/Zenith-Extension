...
class GlobTool extends client_1.ClientTool {
    static toolName = 'glob';
    static description = 'Search for files using glob patterns';
    static parameters = zod_1.z.object({
        ...
        path: zod_1.z
            .string()
            .optional()
            .describe('The directory to search in. If not specified, the current working directory will be used. Must be a valid directory path if provided.'),
...
async function tryGlobApproaches(sandbox, searchPath, pattern) {
...
async function tryBashGlob(sandbox, searchPath, pattern) {
    try {
        const fullPattern = buildFullPattern(searchPath, pattern);
        const exclusions = (0, cli_1.buildShellExclusionPattern)();
...
async function tryShGlob(sandbox, searchPath, pattern) {
    try {
        const fullPattern = buildFullPattern(searchPath, pattern);
        const exclusions = (0, cli_1.buildShellExclusionPattern)();
...
async function tryFindGlob(sandbox, searchPath, pattern) {
    try {
...
function buildFullPattern(searchPath, pattern) {
    if (searchPath === '.') {
        return pattern;
    }
...
async function validateInputs(pattern, searchPath, sandbox) {
...
async function processAndFormatResults(output, pattern, searchPath, method) {
    if (!output || !output.trim()) {
        return `No files found matching pattern "${pattern}" in path "${searchPath}"`;
    }
    let lines = output.split('\n').map(line => line.trim()).filter(line => line.length > 0);
    // Additional filtering for any paths that slipped through
    lines = (0, cli_1.filterExcludedPaths)(lines);
    // Clean up the output
...
function extractPatternBasePath(pattern, searchPath) {
