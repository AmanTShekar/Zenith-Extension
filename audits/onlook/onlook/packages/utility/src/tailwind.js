...
exports.CssToTailwindTranslator = exports.defaultTranslatorConfig = exports.propertyMap = exports.specialAttribute = void 0;
exports.specialAttribute = ['@charset', '@font-face', '@import', '@keyframes'];
let useAllDefaultValues = false;
let customTheme = {};
const hasNegative = (val) => [
    val[0] === '-' ? '-' : '',
...
exports.propertyMap = new Map([
    ...
    [
        ...
        (val) => {
            ...
            const backdropFilterValConfig = {
                blur: (v) => `backdrop-blur-${customTheme['backdrop-blur']?.[v] ?? `[${v}]`}`,
                brightness: (v) => `backdrop-brightness-${customTheme['backdrop-brightness']?.[v] ?? `[${v}]`}`,
                contrast: (v) => `backdrop-contrast-${customTheme['backdrop-contrast']?.[v] ?? `[${v}]`}`,
                grayscale: (v) => `backdrop-grayscale-${customTheme['backdrop-grayscale']?.[v] ?? `[${v}]`}`,
                'hue-rotate': (v) => {
                    const t = hasNegative(v);
                    return `${t[0]}backdrop-hue-rotate-${customTheme['backdrop-grayscale']?.[t[1]] ?? `[${t[1]}]`}`;
                },
                invert: (v) => `backdrop-invert-${customTheme['backdrop-invert']?.[v] ?? `[${v}]`}`,
                opacity: (v) => `backdrop-opacity-${customTheme['backdrop-opacity']?.[v] ?? `[${v}]`}`,
                saturate: (v) => `backdrop-saturate-${customTheme['backdrop-saturate']?.[v] ?? `[${v}]`}`,
                sepia: (v) => `backdrop-sepia-${customTheme['backdrop-sepia']?.[v] ?? `[${v}]`}`,
            };
    ...
    [
        ...
        (val) => {
            const legalConfig = {
                ...exports.propertyMap.get('background-attachment'),
                ...exports.propertyMap.get('background-repeat'),
    ...
    [
        ...
        (val) => {
            ...
            const vals = val
                .split(' ')
                .filter((v) => v !== '')
                .map((v) => isUnit(v) || isColor(v)
                ? ({
                    transparent: 'border-transparent',
                    currentColor: 'border-current',
                    currentcolor: 'border-current',
                }[val] ??
                    exports.propertyMap.get('border-style')[v] ??
                    `border-[${v}]`)
                : (exports.propertyMap.get('border-style')[v] ??
                    ''))
                .filter((v) => v !== '')
    ...
    [
        ...
        (val) => ({ '0': 'rounded-bl-none', '0px': 'rounded-bl-none' })[val] ??
            (isUnit(val)
    ...
    [
        ...
        (val) => ({ '0': 'rounded-br-none', '0px': 'rounded-br-none' })[val] ??
            (isUnit(val)
    ...
    [
        'border-bottom-style',
        (val) => exports.propertyMap.get('border-style')[val]
    ...
    [
        ...
        (val) => ({ '0': 'rounded-tl-none', '0px': 'rounded-tl-none' })[val] ??
            (isUnit(val)
    ...
    [
        ...
        (val) => ({ '0': 'rounded-tr-none', '0px': 'rounded-tr-none' })[val] ??
            (isUnit(val)
    ...
    [
        'fill',
        (val, isCustom = false) => ({ currentColor: 'fill-current', currentcolor: 'fill-current' })[val] ??
    ...
    [
        ...
        (val) => {
            ...
            const filterValConfig = {
                blur: (v) => `blur-${customTheme['blur']?.[v] ?? `[${v}]`}`,
                brightness: (v) => `brightness-${customTheme['brightness']?.[v] ?? `[${v}]`}`,
                contrast: (v) => `contrast-${customTheme['contrast']?.[v] ?? `[${v}]`}`,
                grayscale: (v) => `grayscale-${customTheme['grayscale']?.[v] ?? `[${v}]`}`,
                'hue-rotate': (v) => {
                    const t = hasNegative(v);
                    return `${t[0]}hue-rotate-${customTheme['grayscale']?.[t[1]] ?? `[${t[1]}]`}`;
                },
                invert: (v) => `invert-${customTheme['invert']?.[v] ?? `[${v}]`}`,
                saturate: (v) => `saturate-${customTheme['saturate']?.[v] ?? `[${v}]`}`,
                sepia: (v) => `sepia-${customTheme['sepia']?.[v] ?? `[${v}]`}`,
            };
            const vals = getCustomVal(val)
                .replace(/\(.+?\)/g, (v) => v.replace(/_/g, ''))
                .split(')_')
    ...
    [
        ...
        (val) => {
            const t = hasNegative(val);
            return ({ '0': 'mb-0', '0px': 'mb-0', auto: 'mb-auto' }[val] ??
                (isUnit(val)
    ...
    [
        ...
        (val) => {
            const t = hasNegative(val);
            return ({ '0': 'ml-0', '0px': 'ml-0', auto: 'ml-auto' }[val] ??
                (isUnit(val)
    ...
    [
        ...
        (val) => {
            const t = hasNegative(val);
            return ({ '0': 'mr-0', '0px': 'mr-0', auto: 'mr-auto' }[val] ??
                (isUnit(val)
    ...
    [
        ...
        (val) => {
            const t = hasNegative(val);
            return ({ '0': 'mt-0', '0px': 'mt-0', auto: 'mt-auto' }[val] ??
                (isUnit(val)
    ...
    [
        ...
        (val) => {
            ...
            const transformValConfig = {
                ...
                scaleX: (v) => `scale-x-${customTheme.scale?.[v] || (useAllDefaultValues && scaleDefaultVs[v]) || `[${v}]`}`,
                scaleY: (v) => `scale-y-${customTheme.scale?.[v] || (useAllDefaultValues && scaleDefaultVs[v]) || `[${v}]`}`,
                rotate: (v) => {
                    const vs = v.split(',');
                    if (vs.length > 1) {
                        if (vs.length === 3 &&
...
const getResultCode = (it, prefix = '', config) => {
    ...
    const resultVals = cssCodeList
        ...
        const pipe = exports.propertyMap.get(key.trim());
        ...
        if (it.selectorName.endsWith(':hover') && pipeVal.length > 0) {
            if (['backdrop-filter', 'filter', 'transform'].filter((v) => pipeVal.startsWith(v))
                .length > 0) {
...
exports.defaultTranslatorConfig = {
    prefix: '',
    useAllDefaultValues: true,
    customTheme: {},
};
const CssToTailwindTranslator = (code, config = exports.defaultTranslatorConfig) => {
    if (exports.specialAttribute.map((v) => code.includes(v)).filter((v) => v).length > 0) {
        return {
            code: 'SyntaxError',
            data: [],
        };
    }
    useAllDefaultValues = config.useAllDefaultValues ?? exports.defaultTranslatorConfig.useAllDefaultValues;
    customTheme = config.customTheme ?? exports.defaultTranslatorConfig.customTheme;
    const dataArray = [];
    parsingCode(code)
        .map((it) => {
        if (typeof it.cssCode === 'string') {
            ...
            return it.cssCode.map((v) => {
                const mediaName = getCustomVal(it.selectorName
                    .replace(/\(.+\)/g, (v) => v.replace(/\s/g, ''))
                    .replace(/\s+\(/g, '('));
                const res = getResultCode(v, customTheme.media?.[it.selectorName] ||
...
exports.CssToTailwindTranslator = CssToTailwindTranslator;
