...
exports.isColorEmpty = isColorEmpty;
exports.formatHexString = formatHexString;
exports.parseHslValue = parseHslValue;
exports.parseOklchValue = parseOklchValue;
const color_namer_1 = __importDefault(require("color-namer"));
const culori_1 = require("culori");
const parse_css_color_1 = __importDefault(require("parse-css-color"));
const math_1 = require("./math");
function isColorEmpty(colorValue) {
...
function parseHslValue(value) {
    ...
    if (value.includes('hsl')) {
        const hslMatch = value.match(/hsla?\(\s*([^,\s]+)(?:deg)?\s*[,\s]\s*([^,\s]+)%\s*[,\s]\s*([^,\s]+)%\s*(?:[,/]\s*([^)]+))?\s*\)/);
        if (hslMatch) {
...
function parseOklchValue(value) {
    ...
    if (value.includes('oklch')) {
        const oklchMatch = value.match(/oklch\(\s*([^,\s]+)%?\s*[,\s]\s*([^,\s]+)\s*[,\s]\s*([^,\s]+)(?:deg)?\s*(?:[,/]\s*([^)]+))?\s*\)/);
        if (oklchMatch) {
...
class Color {
    ...
    static oklch(oklchColor) {
        ...
        const rgbColor = (0, culori_1.rgb)(oklchInput);
        if (!rgbColor) {
    ...
    static from(str) {
        ...
        const color = (0, parse_css_color_1.default)(formatHexString(str));
        if (color) {
            if (color.type === 'rgb') {
                return Color.rgb({
    ...
    static mix(color0, color1, ratio) {
    ...
    isEqual(other) {
        return ((0, math_1.isNearEqual)(this.h, other.h, 0.001) &&
            (0, math_1.isNearEqual)(this.s, other.s, 0.001) &&
            (0, math_1.isNearEqual)(this.v, other.v, 0.001) &&
            (0, math_1.isNearEqual)(this.a, other.a, 0.001));
    }
    get rgb() {
    ...
    get name() {
        return this._name
            ? this._name
            : ((0, color_namer_1.default)(this.toHex6()).ntc[0]?.name?.toLowerCase().replace(' ', '-') ?? '');
    }
    set name(newName) {
        this._name = newName;
    }
