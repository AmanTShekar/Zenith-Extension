...
function extractFontParts(fileName) {
    ...
    if (weight) {
        let match = fonts_1.VARIANTS.find((variant) => variant.name.toLowerCase() === weight.toLowerCase());
        if (!match && /^\d+$/.test(weight)) {
            match = fonts_1.VARIANTS.find((variant) => variant.value === weight);
        }
        if (!match) {
            ...
            match = fonts_1.VARIANTS.find((variant) => {
            ...
            if (!match) {
                match = fonts_1.VARIANTS.find((variant) => {
                    const variantLower = variant.name.toLowerCase();
                    return (weightLower.includes(variantLower) &&
    ...
    if (!style) {
        style = fonts_1.DEFAULT.STYLE;
    }
    if (!weight) {
        weight = fonts_1.DEFAULT.WEIGHT;
    }
 ...
 */
function getFontFileName(baseName, weight, style) {
    const weightMap = {
        '100': 'Thin',
        '200': 'ExtraLight',
        '300': 'Light',
    ...
    const styleName = style.toLowerCase() === 'normal' ? '' : style.charAt(0).toUpperCase() + style.slice(1);
 ...
 */
function convertFontString(fontString) {
    if (!fontString) {
        return '';
    }
    ...
    return (0, lodash_1.camelCase)(withoutFallback);
}
/**
...
function convertFontWeight(weight) {
    return fonts_1.VARIANTS.find((variant) => variant.value === weight)?.name ?? weight;
}
